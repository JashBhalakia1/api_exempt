          ...
          <div class="h-100 p-5 text-bg-dark rounded-3">
            ...
            <form action="<?php print basename($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multipart/form-data">
                <div class="mb-3">
					...
					<input type="text" name="fullname" class="form-control form-control-lg" maxlength="50" id="fullname" placeholder="Enter your Full Name" <?php print (isset($_SESSION["fullname"])) ? 'value="'.$_SESSION["fullname"].'"'  : ''; unset($_SESSION["fullname"]); ?> >
                ...
                <button type="submit" name="signup" class="btn btn-primary">Enter</button>
              </form>
          </div>
        </div>
<?php
      ...
      <div class="row align-items-md-stretch">
        <div class="col-md-9">
          <div class="h-100 p-5 text-bg-dark rounded-3">
            ...
            <form action="<?php print basename($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multipart/form-data">
                <div class="mb-3">
                    <label for="fullname" class="form-label">Verification of the Code:</label>
      ...
      <div class="row align-items-md-stretch">
        <div class="col-md-9">
          <div class="h-100 p-5 text-bg-dark rounded-3">
            ...
            <form action="<?php print basename($_SERVER["PHP_SELF"]); ?>" method="POST" enctype="multipart/form-data">
                <div class="mb-3">
                    <label for="passphrase" class="form-label">Enter your own password:</label>
                  ...
                  <div class="mb-3">
                    <label for="conf_passphrase" class="form-label">Confirm the password:</label>
                ...
                <button type="submit" name="set_pass" class="btn btn-primary">Save the Password</button>
            </form>
          </div>
        </div>
<?php
...
$spot_profile = $conn->select(sprintf("SELECT `users`.`userId`, `users`.`fullname`, `users`.`email`, `users`.`username`, `users`.`genderId`, `users`.`roleId`, `gender`.`gender`, `roles`.`role` FROM `users` LEFT JOIN gender USING(genderId) LEFT JOIN roles USING(roleId) WHERE `users`.`userId` = '%d' LIMIT 1", $_SESSION['consort']['userId']));
            ...
            <div class="mb-3">
              ...
              <div class="mb-3">
                ...
                <div class="mb-3">
                  <label for="roleId" class="form-label">Your Role:</label>
                  <select name="roleId" id="roleId" class="form-control form-control-lg"  required>
                  <option value="<?php print $spot_profile["roleId"]; ?>"><?php print $spot_profile["role"]; ?></option>
                    ...
                    <button type="submit" name="update_profile" class="btn btn-primary">Update Your Profile</button>
                  </form>
                </div>
              </div>
    <?php
